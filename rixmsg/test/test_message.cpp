...
class MessageBaseTest : public rix::msg::MessageBase {
  public:
    ...
    void test_serialize_custom_vec() {
        ...
        serialize_custom_vec(vec, encoded);
        assert(encoded.size() == 26);
        assert(*reinterpret_cast<const uint32_t*>(encoded.data() + 13) == 3);
        ...
        vec[2].number = 897;
        serialize_custom_vec(vec, encoded);
        assert(encoded.size() == 39);
        assert(*reinterpret_cast<const uint32_t*>(encoded.data() + 26) == 3);
...
void test_other_message() {
    using rix::msg::example::OtherMessage;
    using rix::msg::Serializer;

    OtherMessage msg, msg_copy;
    std::vector<uint8_t> encoded;
    msg.flag = true;
    msg.number = 1234;
    assert(Serializer<OtherMessage>::size(msg) == 3);
    Serializer<OtherMessage>::serialize(msg, encoded);
    assert(encoded.size() == 3);
    ...
    size_t offset = 0;
    Serializer<OtherMessage>::deserialize(msg_copy, encoded, offset);
    assert(offset == 3);
    assert(msg_copy.flag == true);
    ...
    rix::msg::Serializer<OtherMessage>::deserialize(msg_copy, encoded, offset);
    assert(offset == 9);
    assert(msg_copy.flag == true);
...
void test_example_message() {
    ...
    msg.object.number = 456;
    msg.array.push_back(789);
    msg.array.push_back(101112);
    msg.array.push_back(131415);
    msg.static_array[0] = 161718;
    msg.static_array[1] = 192021;
    msg.static_array[2] = 222324;
    ...
    size_t expected = 4 + 
                      ...
                      (4 + (2 + 1) + (2 + 1) + (2 + 1)) + 
                      ((2 + 1) + (2 + 1) + (2 + 1));
    assert(Serializer<ExampleMessage>::size(msg) ==  expected);

    Serializer<ExampleMessage>::serialize(msg, encoded);
    assert(encoded.size() == Serializer<ExampleMessage>::size(msg));

    size_t offset = 0;
    Serializer<ExampleMessage>::deserialize(msg_copy, encoded, offset);
    assert(offset == expected);
    assert(msg_copy.number == 123);
    assert(msg_copy.word == "Hello, world!");
    assert(msg_copy.flag == true);
    assert(msg_copy.object.flag == false);
    assert(msg_copy.object.number == 456);
    assert(msg_copy.array.size() == 3);
    assert(msg_copy.array[0] == 789);
    assert(msg_copy.array[1] == 101112);
    assert(msg_copy.array[2] == 131415);
    assert(msg_copy.static_array[0] == 161718);
    assert(msg_copy.static_array[1] == 192021);
    assert(msg_copy.static_array[2] == 222324);
    ...
    assert(msg_copy.array_of_objects[0].number == 456);
    assert(msg_copy.array_of_objects[1].flag == false);
    assert(msg_copy.array_of_objects[1].number == 456);
    assert(msg_copy.array_of_objects[2].flag == false);
    assert(msg_copy.array_of_objects[2].number == 456);
    assert(msg_copy.static_array_of_objects[0].flag == false);
    assert(msg_copy.static_array_of_objects[0].number == 456);
    assert(msg_copy.static_array_of_objects[1].flag == false);
    assert(msg_copy.static_array_of_objects[1].number == 456);
    assert(msg_copy.static_array_of_objects[2].flag == false);
    assert(msg_copy.static_array_of_objects[2].number == 456);
}
