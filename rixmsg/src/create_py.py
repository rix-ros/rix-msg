...
def create_rixmsg_py_constructor(fields: list) -> str:
    ...
    for field in fields:
        ...
        if is_map and not key_is_base:
            raise ValueError(f"Error: Value type for associative container cannot be custom type {field['type']}")
        ...
        if value_is_base:
            ...
            # If we have a single base type
                ...
                fields_str += f"self.{field['name']} = {PY_TYPE_INITIALIZERS[value_type]}\n"
    ...
    for field in fields:
        ...
        if value_type == "string":
            if is_dynamic_arr:
                ...
                serialize_str += f"Message._serialize_string(self.{field['name']}, buffer)\n"
        elif value_is_base:
            if is_dynamic_arr:
                ...
                serialize_str += f"Message._serialize_{value_type}(self.{field['name']}, buffer)\n"
        elif 'package' in field:
            if is_dynamic_arr:
                ...
                serialize_str += f"Message._serialize_message(self.{field['name']}, buffer)\n"
        else:
            raise ValueError(f"Error: No package specified for type {field['type']}")
...
#     return f"""from rixmsg.message_base import Message
