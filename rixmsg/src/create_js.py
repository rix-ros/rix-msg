    ...
    for field in fields:
        ...
        # If we have a message type (package specified)
            ...
            # If we have a single message type
                ...
                fields_str += f"this.{field['name']} = new {value_type}();\n"
    ...
    for field in fields:
        ...
        if value_type == "string":
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    ...
                    size_str += f"size += Message._size_map_number_to_string(this.{field['name']}, Message._size_{key_type}())\n"
            else:
        ...
        # If we have a base type other than a string
            ...
            # If we have a static array of base types
                ...
                if key_type == "string":
                    size_str += f"size += Message._size_map_string_to_number(this.{field['name']}, Message._size_{value_type}())\n"
                else:
                    size_str += f"size += Message._size_map_number_to_number(this.{field['name']}, Message._size_{key_type}(), Message._size_{value_type}())\n"
            else:
        ...
        # If we have a message type (package specified)
            ...
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    ...
                    size_str += f"size += Message._size_map_number_to_message(this.{field['name']}, Message._size_{key_type}())\n"
            else:
        ...
        if value_type == "string":
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    ...
                    serialize_str += f"buffer = Message._serialize_map(this.{field['name']}, buffer, Message._serialize_{key_type}, Message._serialize_string)\n"
            else:
                serialize_str += f"buffer = Message._serialize_string(this.{field['name']}, buffer);\n"
        elif value_is_base:
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    serialize_str += f"buffer = Message._serialize_map(this.{field['name']}, buffer, Message._serialize_string, Message._serialize_{value_type})\n"
                else:
            ...
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    ...
                    serialize_str += f"buffer = Message._serialize_map(this.{field['name']}, buffer, Message._serialize_{key_type}, Message._serialize_message)\n"
            else:
        ...
        if value_type == "string":
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    ...
                    deserialize_str += f"this.{field['name']} = Message._deserialize_map(buffer, context, Message._deserialize_{key_type}, Message._deserialize_string)\n"
            else:
            ...
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    deserialize_str += f"this.{field['name']} = Message._deserialize_map(buffer, context, Message._deserialize_string, Message._deserialize_{value_type})\n"
                else:
            ...
            if is_dynamic_arr:
                ...
                if key_type == "string":
                    deserialize_str += f"this.{field['name']} = Message._deserialize_map(buffer, context, Message._deserialize_string, Message._deserialize_message, {value_type})\n"
                else:
                    deserialize_str += f"this.{field['name']} = Message._deserialize_map(buffer, context, Message._deserialize_{key_type}, Message._deserialize_message, {value_type})\n"
            else:
